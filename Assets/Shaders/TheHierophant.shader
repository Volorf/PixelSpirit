Shader "Pixel Spirit/TheHierophant"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float rect = rectSDF(i.uv.xy * 2 - 0.5, float2(1.0, 1.0));
                float fac = fill(rect, 0.5);
                float cross = crossSDF(i.uv.xy * 2 - 0.5, 1.0);
                fac *= step(0.5, frac(cross * 4.0));
                fac *= step(1.0, cross);
                fac += fill(cross, 0.5);
                fac += stroke(rect, 0.65, 0.05);
                fac += stroke(rect, 0.75, 0.025);
